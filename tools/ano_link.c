...
void ANO_DT_Data_Receive_Anl(uint8_t *data_buf, uint8_t num)
{
    ...
    if (*(data_buf + 2) == 0x12) //接收PID3
    {
        ...
        Total_Controller.High_Speed_Control.Kd = 0.01 * ((int16_t)(*(data_buf + 8) << 8) | *(data_buf + 9));
        ...
        Total_Controller.High_Position_Control.Kd = 0.01 * ((int16_t)(*(data_buf + 14) << 8) | *(data_buf + 15));
        ANO_DT_Send_Check(*(data_buf + 2), sum);
    }
...
* 注    意：none
********************************************/
void ANO_DT_Send_High(int pressure_high, uint16_t ultrasonic_high)
{
    uint8_t _cnt = 0;
    int _temp;
    uint16_t _temp2;
...
void ANO_SEND_StateMachine(void)
{
    ...
    if (ANO_Cnt == 1)
    {
        ANO_Data_Send_Status(rovInfo.jy901.roll, rovInfo.jy901.pitch, rovInfo.jy901.yaw, rovInfo.depthSensor.depth); //发送基本信息（欧拉三角、高度、锁定状态）【第二组】
    }

    else if (ANO_Cnt == 2)
    { //发送传感器原始数字量 (加速度、角速度、磁场)  【第三组】
    ...
    {
        ANO_DT_Send_High(rovInfo.depthSensor.pressure, 0); //发送高度数据 (气压计高度、超声波高低)
    }

    else if (ANO_Cnt == 6                                                                                                                                                                   //发送PID数据
             && ANO_Send_PID_Flag[0] == 0 && ANO_Send_PID_Flag[1] == 0 && ANO_Send_PID_Flag[2] == 0 && ANO_Send_PID_Flag[3] == 0 && ANO_Send_PID_Flag[4] == 0 && ANO_Send_PID_Flag[5] == 0) //提前终止发送队列
    {
        ANO_Cnt = 0;
    }

    else if (ANO_Cnt == 7 && ANO_Send_PID_Flag[0] == 1)
    { //第1帧 PID
